                        ...
                        <div class="list-group list-group-flush">
                            @if ($allUsers->isEmpty())
                                <div class="text-center text-muted py-3">
                                    <p>No messages to display. Start a conversation!</p>
                                </div>
                            @else
                                    ...
                                    <a href="{{ route('messages.show', $user->username) }}"
                                        ...
                                        @if ($user->profile_picture)
                                            <img src="{{ asset('storage/' . $user->profile_picture) }}"
                                                alt="Profile Picture" class="img-fluid rounded-circle me-2"
                                                width="50" height="50">
                                        @else
                                            <img src="{{ asset('/default-images/avatar.png') }}"
                                                alt="Default Profile Picture" class="img-fluid rounded-circle me-2"
                                                width="50" height="50">
                                        @endif
                                        <div>
                                            ...
                                            <div class="text-muted small">
                                                {{ Str::limit(optional($user->lastMessage)->content, 30) }}</div>
                            ...
                            <div class="card-header d-flex align-items-center">
                                @if ($recipient->profile_picture)
                                    <img src="{{ asset('storage/' . $recipient->profile_picture) }}"
                                        alt="Profile Picture" class="img-fluid rounded-circle me-2" width="50"
                                        height="50">
                                @else
                                    <img src="{{ asset('/default-images/avatar.png') }}" alt="Default Profile Picture"
                                        class="img-fluid rounded-circle me-2" width="50" height="50">
                                @endif
                                <div>
                                    <h5 class="card-title mb-0">
                                        @if ($recipient->profile_type == 'institution')
                                            {{ $recipient->institutionDetails->institution_name }}
                                        @else
                            ...
                            <div class="card-body">
                                ...
                                <div class="mb-3">
                                    @foreach ($messages as $message)
                                        <div
                                            class="d-flex {{ $message->sender_id == Auth::id() ? 'justify-content-end' : '' }} mb-2">
                                            <div class="{{ $message->sender_id == Auth::id() ? 'text-end' : '' }}">
                                                <div
                                                    class="p-2 rounded {{ $message->sender_id == Auth::id() ? 'bg-primary text-white' : 'bg-light' }}">
                                                    {{ $message->content }}
                                                </div>
                                                <div class="text-muted small">
                                                    {{ $message->created_at->format('M d, Y, g:i A') }}</div>
                                            </div>
                                        </div>
                                    @endforeach
                                </div>

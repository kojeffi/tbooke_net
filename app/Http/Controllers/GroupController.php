 ...
 // Show a group's details and posts
    ...
    public function storeComment(Request $request, $postId)
    {
       ...
       $comment = GroupComment::create([
        ...
        $post = GroupPost::where('id', $comment->post_id)->first();
        $members = GroupMember::join('users', 'users.id', '=', 'group_members.user_id')
        ->where('group_id', $post->group_id)->get();
        $user = auth()->user();
        foreach ($members as  $member) {
           Mail::to($member->email)->send(new SendToGroup($post, $user,'New  Comment!',3,$member));
        }
    ...
    {
        $like = GroupLike::create([
            'post_id' => $postId,
            'user_id' => auth()->id(),
        ]);

        $post = GroupPost::where('id', $like->post_id)->first();
        $members = GroupMember::join('users', 'users.id', '=', 'group_members.user_id')
        ->where('group_id', $post->group_id)->get();
        $user = auth()->user();
        //dd($post);
        foreach ($members as  $member) {
           Mail::to($member->email)->send(new SendToGroup($post, $user,'New Like!',2,$member));
        }
    ...
    public function repostPost(Request $request, $postId)
    {
        ...
        $group =  GroupRepost::create([
            'post_id' => $postId,
            'user_id' => auth()->id(),
            'content' => $request->content,
        ]);
        //dd($group);
        $post = GroupPost::where('id', $group->post_id)->first();
        $members = GroupMember::join('users', 'users.id', '=', 'group_members.user_id')
        ->where('group_id', $post->group_id)->get();
        $user = auth()->user();
        foreach ($members as  $member) {
           Mail::to($member->email)->send(new SendToGroup($post, $user,'New Reposted!',1,$member));
        }
